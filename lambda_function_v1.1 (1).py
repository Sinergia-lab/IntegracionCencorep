...
class Odoo:
    ...
    def revisarDF_TXD(self,dataframe,tempo):
        if dataframe.tipo=='producto_TXD':
            ...
            df['UN_ON_LINE']=df['UN_ON_LINE'].astype(int)
            df['UNID_VEND']=df['UNID_VEND'].astype(int)
            largo=len(df)
            avance25=1
            avance50=1
            avance75=1
            for fila in range(largo):
                if tempo.verificarTiempoLimite(time.time()):
                    ...
                    skuProducto = datos_fila['SKU']
                    yearVenta,mesVenta = procesar_fecha2( datos_fila['FECHA'] ) 
                    ...
                    if(presParis+ecoParis)>0:
                        ...
                        if not busquedaSKU:
                            prod= Producto(
                                ...
                                '',
                                '',
                                datos_fila['PROVE_RAZON'],
                                datos_fila['DESC_CLASE'],
                                procesar_origen(datos_fila['ORIGEN']),
                                datos_fila['MARCA'],
                                'No completado - Nuevo',
                                datos_fila['DESC_DEPTO'],
    ...
    def revisarDF_TXD_multiple(self,dataframe,tempo):
        def row2prod(datos_fila):
            prod = Producto('TXD',datos_fila['SKU'],'','', datos_fila['PROVE_RAZON'],datos_fila['DESC_CLASE'],
                    procesar_origen(datos_fila['ORIGEN']),datos_fila['MARCA'],'No completado - Nuevo',datos_fila['DESC_DEPTO'],
                    datos_fila['DESC_SUBDEPTO'],'','','','','')
            return prod
        ...
        df = dataframe.dataFrame
        prods_vendidos = df[(df['UNID_VEND']+df['UN_ON_LINE']) > 0]
        prods_nuevos = prods_vendidos.apply(lambda row:not self.buscarProductoRam_fast(row['SKU'],'TXD'), axis=1)
        prods_nuevos = prods_vendidos[prods_nuevos]
        prods_nuevos = list(prods_nuevos.apply(row2prod,axis=1))

        chunk_size = 60
        for i in range(0,len(prods_nuevos),chunk_size):
            self.crearProductos_multiple(prods_nuevos[i:i+chunk_size])

    def procesarDF_multiple(self,dataFrame,aws,tempo):
        if dataFrame.unidadNegocio=='TXD':
            print('iniciando proceso TXD')
    ...
    def crearProductos_multiple(self,lista_productos):   
        ...
        for prod in lista_productos:
            ...
            if prod.unidadNegocio=='TXD':
                ...
                idcategoria2=self.buscarCategoriaRam_fast(self.departamentos_fastseach,prod.categoria2)
                if not idcategoria2:
        ...
        self.productos.create(listado_productos_crear)
        return True
    # FIN NUEVO
